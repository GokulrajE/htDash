...
- **AE / Robot Issue logic** — full detail in `docs/ae_ri_logic.md`. Summary:
  ...
  - **Discontinued patient event visibility** — when `discontinuationDate` is set, only the AE follow-up chain and assessment events are shown in the overdue/upcoming panels. All other incomplete protocol events are hidden (training is over, devices are returned). Server constant `_DISCONTINUED_VISIBLE` drives this filter in both `api_patient_events` and the dashboard events API: `frozenset({'adverse_event', 'adverse_event_followup', 'adverse_event_followup_visit', 'adverse_event_clinical_visit', 'a1_assessment', 'a2_assessment'})`. RI/ODI chains are not included — devices are already returned at discontinuation. This filter applies in addition to the read-only banner and 403 guards on complete-event routes.
  - **RI / ODI visit modals in broken_protocol** — when patient status is `broken_protocol`, the `robot_issue_visit`, `resolve_robot_issue_visit`, and `other_device_issue_visit` modals switch to a simplified **fault-documentation-only mode**: outcome options (Repaired / Swapped / Neither) are removed; the engineer records which devices are faulty and a fault description; `has_issue: true` is set and a `faulty` device event is logged; device assignments are NOT closed (they stay open until the discontinuation modal is completed).
  - **Device assignment removal** — device assignments are closed exclusively when the discontinuation modal is completed (server writes `end_date` to all open assignments and clears `has_issue` flags). No other event or route closes assignments. This applies to all device types: Pluto, Mars, AGWatch, modem, laptop, SIM.
  - **"Discontinue Patient" button visibility** — button is hidden when: (a) patient is `broken_protocol` (those patients use the `discontinuation_reminder` synthetic event), (b) a real `discontinuation` stub already exists in `incomplete`, or (c) patient is already `discontinued`, `all_completed`, or `pre_discontinued`. Button is visible for: `inactive`, `active`, `paused`, `training_completed`, `a1_completed`. Clicking the button triggers a double confirmation popup; on confirm the server creates a `discontinuation` stub in `incomplete` and the button disappears.
