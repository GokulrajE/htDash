...
- **`resolve_robot_issue_visit` stub:** created when a `robot_issue_visit` swaps a device with no replacement available (`new_device_id: null`). The faulty device is taken back and marked faulty in inventory; patient has no device; training paused. No explicit `paused` field — pause is derived. Resolved by engineer/admin: brings replacement device(s); assigns new device per taken-back device; records `can_resume_from`. If null is selected again, another stub is created. Pause clears only when no `resolve_robot_issue_visit` stubs remain AND no `adverse_event_followup` stubs remain.
