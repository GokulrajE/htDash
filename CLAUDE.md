...
- Free event types: `patient_call`, `adverse_event`, `adverse_event_followup`, `robot_issue` (experimental only), `resolve_robot_issue` (experimental only), `watch_record`, `discontinuation`. `technical_fault` was renamed to `robot_issue` — do not use the old name anywhere.
