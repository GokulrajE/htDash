...
- **Assessment scheduling calls (`schedule_a1_call`, `schedule_a2_call`):**
  ...
  - **Auto-seeding (both A1 and A2):** `api_patient_events` lazily seeds a stub when the corresponding assessment is in `incomplete` with `appointment_date = null` AND no scheduling stub exists. For A2 only: also seeded 7 days before the A2 window opens regardless of `appointment_date` state. No "Schedule Call" button on event rows — stubs are always server-seeded.
